    ...
    public ArrayList<Rater> loadRaters(String filename){
        ...
        String rating = record.get("rating");
            if (map.keySet().contains(rater_id)){
                ...
                map.put(rater_id, new Rater(rater_id));
                map.get(rater_id).addRating(movie_id, Double.parseDouble(rating));
            }
        }
        ArrayList<Rater> rater = new ArrayList(map.values());
        return rater;
    }
    
    public void testLoadRaters(){
        ArrayList<Rater> data = loadRaters("data/ratings.csv");
        ...
        for (Rater x : data){
            if (x.numRatings()==max_rates){System.out.println(x.getID());}
        }
    }
